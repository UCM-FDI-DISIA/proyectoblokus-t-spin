...
public class FichasPanel extends JPanel implements GameObserver{
	...
	public void updateIcono(int d) {
		...
		if(d==0) {
			 ...
			 for(int i=0;i<21;i++) {
				 botones.get(i).setIcon(new ImageIcon(((new ImageIcon("resources/FichasRojas/"+i+".png")).getImage()).getScaledInstance(150, 130, java.awt.Image.SCALE_SMOOTH)));
					 forma.setPreferredSize(new Dimension(200,180));
			         pn.add(botones.get(i));
				}
		...
		if(d==1) {
			...
			for(int i=0;i<21;i++) {
				botones.get(i).setIcon(new ImageIcon(((new ImageIcon("resources/FichasAzules/"+i+".png")).getImage()).getScaledInstance(150, 130, java.awt.Image.SCALE_SMOOTH)));
				 forma.setPreferredSize(new Dimension(200,180));
		         pn.add(botones.get(i));
			}
		...
		if(d==3) {
			...
			for(int i=0;i<21;i++) {
				botones.get(i).setIcon(new ImageIcon(((new ImageIcon("resources/FichasVerdes/"+i+".png")).getImage()).getScaledInstance(150, 130, java.awt.Image.SCALE_SMOOTH)));
				 forma.setPreferredSize(new Dimension(200,180));
		         pn.add(botones.get(i));
			}
